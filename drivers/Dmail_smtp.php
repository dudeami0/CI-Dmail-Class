 ...
 */
class Dmail_smtp extends CI_Driver {
	...
	function send() {
		...
		if ($this->smtp_vars['host'] == '') {
			...
			// Clear the 220 sent, if there is one

			$this->_send_command('from', $this->sender);
			...
			$this->_send_data($this->_headers . preg_replace('/^\./m', '..$1', $this->_message) . '.');
